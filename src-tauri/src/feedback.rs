...
fn feedback_api_bases() -> [&'static str; 2] {
    [RU_API, PRIMARY_API]
}
...
fn sync_feedback_to_server(feedback: &AppFeedback) -> Result<(), String> {
    ...
    for base in feedback_api_bases() {
        let url = format!("{}/api/feedback", base.trim_end_matches('/'));
        match client.post(&url).json(&payload).send() {
            Ok(resp) => {
                ...
                last_err = format!("{base}: HTTP {}: {}", status.as_u16(), text);
            }
            Err(e) => {
...
fn sync_diagnostic_to_server(diagnostic: &AppDiagnostic) -> Result<(), String> {
    ...
    for base in feedback_api_bases() {
        let url = format!("{}/api/diagnostics/report", base.trim_end_matches('/'));
        match client.post(&url).json(&payload).send() {
            Ok(resp) => {
                let status = resp.status();
                let text = resp.text().unwrap_or_default();
